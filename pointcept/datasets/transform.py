...
class NormalizeColor(object):
    def __call__(self, data_dict):
        if "strength" in data_dict.keys():
            ...
            min_val = strength.min() 
            max_val = strength.max()  
            data_dict["strength"] = (strength - min_val) / (max_val - min_val) - 0.5 
        return data_dict
...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and np.random.rand() < self.p:
            lo = np.min(data_dict["color"], keepdims=True)
            hi = np.max(data_dict["color"], keepdims=True)
            scale = 255 / (hi - lo)
            contrast_feat = (data_dict["color"] - lo) * scale
            blend_factor = (
                np.random.rand() if self.blend_factor is None else self.blend_factor
            )
            data_dict["color"] = (1 - blend_factor) * data_dict["color"] + blend_factor * contrast_feat
        return data_dict
...
class ChromaticTranslation(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and np.random.rand() < self.p:
            tr = (np.random.rand() - 0.5) * 255 * 2 * self.ratio
            data_dict["color"] = np.clip(tr + data_dict["color"], 0, 255)  # 处理单通道
        return data_dict
...
class ChromaticJitter(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and np.random.rand() < self.p:
            noise = np.random.randn(data_dict["color"].shape[0], 1)
            noise *= self.std * 255
            data_dict["color"] = np.clip(
                noise + data_dict["color"], 0, 255
            )
        return data_dict
...
#############################################################
###|----GridSample function for semantic segmentation----|###
#############################################################
@TRANSFORMS.register_module()
...
#############################################################
###|----GridSample function for instance segmentation----|###
#############################################################
# @TRANSFORMS.register_module()
# class GridSample(object):
#     def __init__(
...
#                 data_dict["min_coord"] = min_coord.reshape([1, 3])
#             if self.return_displacement:
#                 displacement = scaled_coord - grid_coord - 0.5  # [-0.5,0.5]
...
#                 data_dict[key_] = data_dict[key_][idx_unique]
...
class SphereCrop(object):
    ...
    def __call__(self, data_dict):
        total_points = data_dict["coord"].shape[0]
        point_max = min(
            int(self.sample_rate * total_points) if self.sample_rate is not None else self.point_max,
            total_points
        )
        ...
        if self.mode == "all":
            if "index" not in data_dict.keys():
                data_dict["index"] = np.arange(total_points)
            data_part_list = []
            if total_points > point_max:
                coord_p, idx_uni = np.random.rand(total_points) * 1e-3, np.array([])
                while idx_uni.size != data_dict["index"].shape[0]:
                    ...
                    data_crop_dict = dict()
                    for key in ['coord', 'grid_coord', 'displacement']:
                        if key in data_dict:
                ...
                data_crop_dict["weight"] = np.zeros(total_points)
                data_crop_dict["index"] = data_dict["index"]
                data_part_list.append(data_crop_dict)
            return data_part_list
        else:  # mode is "random" or "center"
            if total_points > point_max:
                if self.mode == "random":
                    center = data_dict["coord"][np.random.randint(total_points)]
                elif self.mode == "center":
                    center = data_dict["coord"][total_points // 2]
                idx_crop = np.argsort(np.sum(np.square(data_dict["coord"] - center), 1))[:point_max]
                for key in ['coord', 'origin_coord', 'grid_coord', 'segment', 'instance', 'displacement']:
                    if key in data_dict:
...
class Compose(object):
    ...
    def __call__(self, data_dict):
        for t in self.transforms:
            ...
            # Debugging statement to print the keys after each transform
            # print(f"After {t.__class__.__name__}: {data_dict.keys()}")
        return data_dict
